from django.db import models
import random
from django.urls import reverse
from django.utils import timezone
import time
from django.contrib.auth.models import AbstractUser

class User(AbstractUser):
    thumb = models.ImageField()
    emp_id = models.CharField(max_length=15) 

# Create your models here.
class Department(models.Model):
    name = models.CharField(max_length=70, null=False, blank=False)
    history = models.TextField(max_length=1000,null=True,blank=True, default='No History')
    

    def __str__(self):
        return self.name

    def get_absolute_url(self):
        return reverse("hrms:dept_detail", kwargs={"pk": self.pk})
    

class Employee(models.Model):
    STAFF = (('','SELECT'), ('0','ADMIN'), ('1', 'EMPLOYEE'))
    LANGUAGE = (('english','ENGLISH'),('yoruba','YORUBA'),('hausa','HAUSA'),('french','FRENCH'))
    GENDER = (('male','MALE'), ('female', 'FEMALE'),('other', 'OTHER'))
    EMPLOYEE_TYPE = (('full_time', 'Full Time'), ('consultant','Consultant'), ('contract', 'Contract'))
    #emp_id = models.CharField(max_length=70, default='emp'+str(random.randrange(100,999,1)))
    emp_id = models.CharField(max_length=50, null=False)
    thumb = models.ImageField(blank=True,null=True)
    first_name = models.CharField(max_length=50, null=False)
    last_name = models.CharField(max_length=50, null=False)
    employee_type = models.CharField(choices=EMPLOYEE_TYPE, max_length=20)
    mobile = models.CharField(max_length=15)
    email = models.EmailField(max_length=125, null=False)
    address = models.TextField(max_length=100, default='')
    department = models.ForeignKey(Department,on_delete=models.SET_NULL, null=True)
    designation = models.CharField(max_length=30, null=False)
    reporting_officer = models.CharField(max_length=30, null=False)
    shift = models.CharField(max_length=30, null=False)
    experience = models.CharField(max_length=30, null=False)
    #date_leave = models.CharField(blank=False, max_length=15)
    dob = models.CharField(blank=False, max_length=15)
    gender = models.CharField(choices=GENDER, max_length=10)
    qualification = models.CharField(max_length=50, null=False)
    merital_status = models.CharField(max_length=50, null=False)
    place_of_birth = models.CharField(max_length=50, null=False)
    blood_group = models.CharField(max_length=50, null=False)
    emergency = models.CharField(max_length=11)
    emergency_contact_person_name = models.CharField(max_length=250)
    relationship = models.CharField(max_length=50)
    personal_mail = models.EmailField(max_length=125, null=False)
    current_address = models.CharField(max_length=300, null=False)
    aadhar_address = models.CharField(max_length=300, null=False)
    uan_number = models.CharField(max_length=15, null=False)
    member_id = models.CharField(max_length=15, null=False)
    esic_number = models.CharField(max_length=15, null=False)
    joined = models.DateTimeField(default=timezone.now)
    language = models.CharField(choices=LANGUAGE, max_length=10, default='english')
    bank_account_number = models.CharField(max_length=20, default='0123456789')
    bank_name = models.CharField(max_length=25, default='First Bank Plc')
    ifsc_code = models.CharField(max_length=15, null=False)
    aadhar_number = models.CharField(max_length=15, null=False)
    pan_number = models.CharField(max_length=50, null=False)
    status = models.CharField(max_length=50, null=False)
    #salary = models.CharField(max_length=16,default='00,000.00')     
    def __str__(self):
        return self.first_name
        
    def get_absolute_url(self):
        return reverse("hrms:employee_view", kwargs={"pk": self.pk})
    

class Kin(models.Model):
    first_name = models.CharField(max_length=20)
    last_name = models.CharField(max_length=20)
    address = models.TextField(max_length=100)
    occupation = models.CharField(max_length=20)
    mobile = models.CharField(max_length=15)
    employee = models.OneToOneField(Employee,on_delete=models.CASCADE, blank=False, null=False)

    def __str__(self):
        return self.first_name+'-'+self.last_name
    
    def get_absolute_url(self):
        return reverse("hrms:employee_view",kwargs={'pk':self.employee.pk})
    

class Attendance (models.Model):
    STATUS = (('PRESENT', 'PRESENT'), ('ABSENT', 'ABSENT'),('UNAVAILABLE', 'UNAVAILABLE'))
    #SAT = (('username', 'username'),('abhi', 'ABHI'))
    date = models.DateField(auto_now_add=True)
    first_in = models.TimeField()
    last_out = models.TimeField(null=True)
    status = models.CharField(choices=STATUS, max_length=15 )
    #sat = models.CharField(choices=SAT, max_length=15 )
    staff = models.ForeignKey(Employee, on_delete=models.SET_NULL, null=True)
    
    def save(self,*args, **kwargs):
        self.first_in = timezone.localtime()
        super(Attendance,self).save(*args, **kwargs)
    
    def __str__(self):
        return 'Attendance -> '+str(self.date) + ' -> ' + str(self.staff)

class Leave (models.Model):
    STATUS = (('Apply', 'APPLY'),('approved','APPROVED'),('unapproved','UNAPPROVED'),('decline','DECLINED'))
    employee = models.OneToOneField(Employee, on_delete=models.CASCADE)
    start = models.CharField(blank=False, max_length=15)
    end = models.CharField(blank=False, max_length=15)
    status = models.CharField(choices=STATUS,  default='Not Approved',max_length=15)

    def __str__(self):
        return self.employee + ' ' + self.start

class Recruitment(models.Model):
    first_name = models.CharField(max_length=25)
    last_name= models.CharField(max_length=25)
    position = models.CharField(max_length=15)
    email = models.EmailField(max_length=25)
    phone = models.CharField(max_length=11)

    def __str__(self):
        return self.first_name +' - '+self.position

